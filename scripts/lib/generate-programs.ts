/**
 * Генерация демо-программ. Вынесена из скрипта в библиотеку, чтобы
 * детерминизм можно было проверить тестом, а не запуском процесса.
 *
 * Открытого API с проходными баллами не существует, поэтому проходные в
 * прототипе — тестовые. Каждая запись помечена isDemo: true и показывается
 * в интерфейсе с меткой «Демо». Реальные значения вносятся вручную в
 * data/snapshot/programs.real.json и перекрывают демо.
 */
...
/**
 * Чистая функция: одинаковый seed и одинаковый вход дают побайтово
 * одинаковый результат. Детерминизм закреплён тестом.
 */
export function generateDemoPrograms({
  directions,
  universities,
  seed = SEED,
  ...
  for (const university of pool) {
    ...
    while (codes.size < Math.max(target, codes.size) && guard < 200) {
      guard += 1;
      const prefix = weightedPick(weights, random, usedPrefixes, byPrefix);
      if (!prefix) break;
...
export { SEED as DEMO_SEED, PINNED as PINNED_UNIVERSITIES, EXCLUDE as EXCLUDED_UNIVERSITY_PATTERN };
